+++
title = "WS2HTTP - Day 2: Implementing Message Routing"
date = "2024-09-23"
description = "Progress update on WS2HTTP: a working prototype in just 150 lines of Node.js code. This post explains the server-to-client message routing architecture, supporting both targeted messages to specific WebSocket IDs and broadcasting to everyone connected to a particular URL path."

[taxonomies]
tags = ["tech", "ws2http", "node", "websockets", "messaging", "routing", "architecture"]
+++
...
the design as initially thought up. The sending part has some minor tweaks, mainly regarding
...
### 1. Targeted Messages to Specific IDs

Send a message to specific ID's, this is probably the most boring one. We can set the `id` field,
and thereby send the message only to these particular sockets. If ws2http can't find a socket it'll
trigger an "ERROR" subsequent event which hopefully removes that connection from storage.

### 2. Broadcasting to URL Paths

Send a message to a URL: ws2http accepts every URL and stores it for each connection, we can then send
