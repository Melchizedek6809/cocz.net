...
that I tried things out, so that now I know WHY I don't like certain
parts of the stack. So, in this entry I'll talk about each component,
...
finishing concentrated MVPs as fast as possible to figure out whether the idea
itself is any good.
...
since this is where for me at least the main advantage lies over Next.js. Maybe they've
...
great SQLite bindings. Since I've come to the conclusion that for these super focused MVPs the following stack is the one I'll try next:
...
Apart from that PHP gets ugly really quickly, but as long as the entire project is just a couple thousand lines it seems like quite the alright choice, given that one doesn't need WebSockets.
...
test things out to my heart's content.
...
## Alternative Backend Languages to Consider
...
least when considering the somewhat popular ones. Golang might kinda fit, but you still have a compilation cycle there, and Lua is just a language I do not like (the VM/implementation is absolutely fantastic, but the language...). 

It might be interesting to write Ruby CGI scripts, since Ruby probably has great SQLite bindings, but I'm not sure how big the ecosystem is outside of Rails. Apart from that I guess Racket/CL might be an option, but I'm not sure how well they fit the whole FastCGI architecture, but they probably can't be beat when it comes to iteration times. 

Although I gotta say, having a script running that watches your source dir and rsync's everything to the server on any change feels super satisfying and not really slower than a REPL, with the added benefit that the REPL/Source can't desynchronize during development.